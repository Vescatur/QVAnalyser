...
class StormTool(Tool):
    ...
    def parse_result(self, result):
        ...
        for line in log.splitlines():
            ...
            self.parse_line_with_status(status, line, "bisim", r"States:\s*(\d+)", result, Measurements.STATES_AFTER_BISIMULATION)
            self.parse_line_with_status(status, line, "bisim", r"Transitions:\s*(\d+)", result, Measurements.BRANCHES_AFTER_BISIMULATION)
            self.parse_line_with_status(status, line, "bisim", r"Choices:\s*(\d+)", result, Measurements.TRANSITIONS_AFTER_BISIMULATION)
    ...
    def parse_output_property(self, line, result, measurement):
        ...
        if match:
            ...
            if match_exact:
                result.measurements[measurement] = match_exact.group(1)
            else:
                result.measurements[measurement] = match.group(2)

    def parse_line_with_status(self, status, line, expected_status, regex, result, measurement):
        if status == expected_status:
            match = re.search(regex, line)
            if match:
                result.measurements[measurement] = match.group(1)
